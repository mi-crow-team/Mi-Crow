...
#SBATCH -c 4
#SBATCH --mem=16G
#SBATCH --job-name=debug-bielik-padding-cpu
...
if [[ -z "${HF_TOKEN:-}" ]] && [[ ! -f "${HF_HOME:-$HOME/.cache/huggingface}/token" ]]; then
  ...
  echo "Or submit with: HF_TOKEN=... sbatch slurm/run_debug_bielik_padding_cpu.sh" >&2
  exit 2
fi
...
uv run python -m experiments.scripts.debug_bielik_padding "${ARGS[@]}"
