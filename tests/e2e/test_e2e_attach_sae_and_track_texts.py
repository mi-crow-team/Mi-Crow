...
def test_e2e_sae_attachment_and_text_tracking(trained_sae_setup):
    ...
    k_value = getattr(sae_hook.sae_engine, "top_k", None)
    k_str = f" (k={k_value})" if k_value is not None else ""
    print(f"✅ TopKSAE loaded: {setup['hidden_dim']} → {sae_hook.context.n_latents} → {setup['hidden_dim']}{k_str}")
    ...
    for i in range(0, len(test_texts), batch_size):
        ...
        model.inference.infer_texts(
            ...
            run_name=None,
            batch_size=None,
            tok_kwargs={"max_length": 64, "padding": True, "truncation": True, "add_special_tokens": True},
            autocast=False,
        )
