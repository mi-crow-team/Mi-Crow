...
def test_e2e_save_inputs_and_outputs(temp_dirs):
    ...
    # Run inference using infer_texts (automatically handles ModelInputDetector setup)
    output, encodings = lm.inference.infer_texts(
        TEST_TEXTS,
        run_name=None,  # We'll save manually after inspection
        batch_size=None,
    ...
    assert output_logits.shape[1] == input_ids.shape[1]  # Same sequence length
    ...
    # Step 6: Save detector outputs to store
    ...
    # Step 7: Verify saved data by loading it back
