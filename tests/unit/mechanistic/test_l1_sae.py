...
# Mock overcomplete module before importing L1Sae
...
overcomplete_module.TopKSAE = MagicMock  # Also needed for __init__.py imports
...
class StubEngine:
    ...
    def encode(self, x):
        # Return (pre_codes, codes) where codes are L1 sparse
...
def test_l1_sae_training_config():
    ...
    # Should inherit from SaeTrainingConfig
...
def test_modify_activations_forward_hook():
    ...
    assert not torch.equal(result, output)  # Should be modified
...
def test_load_backward_compatibility_state_dict(monkeypatch, tmp_path):
    ...
    # This should work but we need metadata, so we'll patch it differently
...
def test_process_activations():
    ...
    sae = make_l1_sae()
    # process_activations should be a no-op since metadata is saved in modify_activations
...
def test_train_method_default_config(monkeypatch):
    ...
    assert call_args[0][3] is not None  # config argument
