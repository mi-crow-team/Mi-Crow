...
class _FakeLM:
    ...
    def get_input_tracker(self):
        """Get the InputTracker instance if it exists."""
        return self._input_tracker
    
    def _ensure_input_tracker(self):
        """Ensure InputTracker exists and return it."""
...
def test_enable_and_disable_text_tracking_positive_max_over_tokens():
    ...
    # InputTracker should be created and enabled
    assert lm.get_input_tracker() is not None
    assert lm.get_input_tracker().enabled

    # Provide texts for a batch of 2 (via InputTracker)
    lm.get_input_tracker().set_current_texts(["hello", "world"])
    ...
    # Update top texts from latents (simulating what SAE hook does)
    # Flatten tensor to [B*T, D] = [4, 3]
    flat_tens = tens.reshape(-1, tens.shape[-1])
    concepts.update_top_texts_from_latents(flat_tens, ["hello", "world"], original_shape=tens.shape)

    # Top texts for neuron 0 should be sorted by descending score
    # New behavior: tracks max activation per text (one entry per text)
    tops0 = concepts.get_top_texts_for_neuron(0)
    # We get: hello (1.0 at token 0), world (0.9 at token 1)
    assert len(tops0) == 2
    ...
    assert len(all_tops) == 3  # 3 neurons total
    ...
    # Disable tracking clears heaps
    concepts.disable_text_tracking()
    assert concepts._top_texts_heaps is None
...
def test_negative_tracking_uses_min_over_tokens_and_asc_sort():
    ...
    lm.get_input_tracker().set_current_texts(["a", "b"])
    tens = torch.tensor([
    ...
    # Update top texts from latents (simulating what SAE hook does)
    flat_tens = tens.reshape(-1, tens.shape[-1])
    concepts.update_top_texts_from_latents(flat_tens, ["a", "b"], original_shape=tens.shape)
