...
class MockTokenizer(PreTrainedTokenizerBase):
    ...
    def convert_ids_to_tokens(self, ids, skip_special_tokens: bool = False):
        ...
        if skip_special_tokens:
            tokens = [
                t for t in tokens if t not in [self._pad_token, self._eos_token, self._bos_token, self._unk_token]
            ]
        if single_input:
    ...
    def decode(self, token_ids: Union[int, List[int], Any], skip_special_tokens: bool = True, **kwargs) -> str:
    ...
    def _encode_plus(
        ...
        **kwargs,
        ...
        result = self.batch_encode_plus(
            texts, add_special_tokens=add_special_tokens, padding=padding, return_tensors=return_tensors, **kwargs
        )
