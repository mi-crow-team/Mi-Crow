...
from mi_crow.language_model.persistence import load_model_from_saved_file, save_model
...
class TestLoadModelFromSavedFile:
    ...
    def test_load_model_success(self, mock_language_model, temp_store, tmpdir):
        ...
        payload = {"model_state_dict": model_state_dict, "metadata": metadata}
        ...
        with patch("mi_crow.language_model.persistence.AutoTokenizer.from_pretrained") as mock_tokenizer:
            with patch("mi_crow.language_model.persistence.AutoModelForCausalLM.from_pretrained") as mock_model:
                ...
                lm = load_model_from_saved_file(type(mock_language_model), saved_path, temp_store, model_id=None)
        ...
        with patch("mi_crow.language_model.persistence.AutoTokenizer.from_pretrained") as mock_tokenizer:
            with patch("mi_crow.language_model.persistence.AutoModelForCausalLM.from_pretrained") as mock_model:
                ...
                lm = load_model_from_saved_file(
                    type(mock_language_model), saved_path, temp_store, model_id="custom_model_id"
                )
    ...
    def test_load_model_file_not_found(self, mock_language_model, temp_store):
        ...
        with pytest.raises(FileNotFoundError, match="Saved model file not found"):
            load_model_from_saved_file(type(mock_language_model), Path("/nonexistent/path/model.pt"), temp_store)
    ...
    def test_load_model_store_none_raises_error(self, mock_language_model, tmpdir):
        ...
        with pytest.raises(ValueError, match="store cannot be None"):
            load_model_from_saved_file(type(mock_language_model), saved_path, None)
    ...
    def test_load_model_invalid_format_missing_state_dict(self, mock_language_model, temp_store, tmpdir):
        ...
        with pytest.raises(ValueError, match="missing 'model_state_dict' key"):
            load_model_from_saved_file(type(mock_language_model), saved_path, temp_store)
    ...
    def test_load_model_no_model_id_in_metadata(self, mock_language_model, temp_store, tmpdir):
        ...
        with pytest.raises(ValueError, match="model_id not found in saved metadata"):
            load_model_from_saved_file(type(mock_language_model), saved_path, temp_store, model_id=None)
    ...
    def test_load_model_huggingface_load_failure(self, mock_language_model, temp_store, tmpdir):
        ...
        torch.save(payload, saved_path)
        with patch(
            "mi_crow.language_model.persistence.AutoTokenizer.from_pretrained", side_effect=Exception("not found")
        ):
