...
class TestLanguageModelActivations:
    ...
    def test_save_activations_dataset_clears_activations_after_save(self, mock_language_model, temp_store):
        ...
        with patch("torch.inference_mode"), patch("torch.cuda.is_available", return_value=False):
            activations.save_activations_dataset(dataset, layer_sig, run_name="test_run", batch_size=2)
    ...
    def test_process_batch_clears_cuda_cache(self, mock_language_model):
        ...
        mock_language_model.save_detector_metadata = Mock()
        with (
            patch.object(torch_module.cuda, "is_available", return_value=True),
    ...
    def test_save_activations_success(self, mock_language_model, temp_store):
        ...
        mock_language_model.save_detector_metadata = Mock()
        with patch("torch.inference_mode"):
            run_name = activations.save_activations(texts, layer_sig, run_name="test_run", batch_size=2, verbose=True)
    ...
    def test_save_activations_multiple_layers(self, mock_language_model, temp_store):
        ...
        mock_language_model.save_detector_metadata = Mock()
        with (
            patch.object(activations, "_setup_detector", return_value=(Mock(), "hook_id")) as mock_setup,
            patch.object(activations, "_cleanup_detector") as mock_cleanup,
    ...
    def test_save_activations_dataset_with_max_length(self, mock_language_model, temp_store):
        ...
        assert "tok_kwargs" in call_kwargs or len(mock_language_model.inference.execute_inference.call_args[0]) > 0
