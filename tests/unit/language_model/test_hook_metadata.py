...
class TestCollectHooksMetadata:
    ...
    def test_collect_single_detector(self):
        ...
        context._hook_registry = {"layer_0": {HookType.FORWARD: [(detector, None)]}}
        result = collect_hooks_metadata(context)
    ...
    def test_collect_single_controller(self):
        ...
        context._hook_registry = {"layer_1": {HookType.PRE_FORWARD: [(controller, None)]}}
        result = collect_hooks_metadata(context)
    ...
    def test_collect_multiple_hooks_same_layer(self):
        ...
        context._hook_registry = {"layer_0": {HookType.FORWARD: [(detector1, None), (detector2, None)]}}
        result = collect_hooks_metadata(context)
        assert "layer_0" in result
    ...
    def test_collect_multiple_layers(self):
        ...
        context._hook_registry = {
            "layer_0": {HookType.FORWARD: [(detector1, None)]},
            "layer_1": {HookType.PRE_FORWARD: [(controller1, None)]},
        }
        result = collect_hooks_metadata(context)
        assert "layer_0" in result
        assert "layer_1" in result
    ...
    def test_collect_with_int_layer_signature(self):
        ...
        detector.enabled = True
        context._hook_registry = {0: {HookType.FORWARD: [(detector, None)]}}
        result = collect_hooks_metadata(context)
        assert "0" in result
