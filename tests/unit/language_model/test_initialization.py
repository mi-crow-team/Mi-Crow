...
from mi_crow.language_model.initialization import create_from_huggingface, create_from_local_torch, initialize_model_id
...
class TestCreateFromHuggingface:
    ...
    def test_create_from_huggingface_success(self, temp_store):
        ...
        with (
            patch("mi_crow.language_model.initialization.AutoTokenizer") as mock_tokenizer_class,
            ...
            lm = create_from_huggingface(LanguageModel, "test/model", temp_store)
    ...
    def test_create_from_huggingface_with_params(self, temp_store):
        ...
        with (
            ...
            lm = create_from_huggingface(
                LanguageModel, "test/model", temp_store, tokenizer_params=tokenizer_params, model_params=model_params
            )
            mock_tokenizer_class.from_pretrained.assert_called_once()
            assert mock_tokenizer_class.from_pretrained.call_args[0][0] == "test/model"
...
class TestCreateFromLocalTorch:
    ...
    def test_create_from_local_torch_success(self, temp_store, tmp_path):
        ...
        with (
            ...
            mock_tokenizer_class.from_pretrained.return_value = mock_tokenizer
            mock_model_class.from_pretrained.return_value = mock_model
            lm = create_from_local_torch(LanguageModel, str(model_path), str(tokenizer_path), temp_store)
            assert lm.model is not None
            mock_model.to.assert_called_once()
            assert lm.tokenizer == mock_tokenizer
            mock_tokenizer_class.from_pretrained.assert_called_once()
