...
def test_from_local_raises_on_missing_text_field_in_structured(tmp_path):
    ...
    with pytest.raises(ValueError):
        TextSnippetDataset.from_local(csv, cache_dir=tmp_path/"cache", text_field="text")
    # But succeeds if specifying correct text_field
    ds = TextSnippetDataset.from_local(csv, cache_dir=tmp_path/"cache2", text_field="msg")
    assert len(ds) == 2
