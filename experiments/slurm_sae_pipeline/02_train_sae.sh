...
#SBATCH --output=/mnt/evafs/groups/mi2lab/akaniasty/Mi-Crow/slurm-logs/sae_train-%j.out
#SBATCH --error=/mnt/evafs/groups/mi2lab/akaniasty/Mi-Crow/slurm-logs/sae_train-%j.err
#SBATCH --export=ALL
...
if [[ -z "${HF_TOKEN:-}" ]] && [[ ! -f "${HF_HOME:-$HOME/.cache/huggingface}/token" ]]; then
  ...
  echo "Or submit with: HF_TOKEN=... sbatch 02_train_sae.sh" >&2
  exit 2
fi

# Run SAE training script
cd "$REPO_DIR/experiments/slurm_sae_pipeline"
$UV_BIN run python 02_train_sae.py --config "$CONFIG_FILE" --run_id "bielik12_polemo2_layer12_3epochs"
