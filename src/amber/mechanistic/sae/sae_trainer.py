...
from amber.mechanistic.sae.training.wandb_logger import WandbLogger
...
class SaeTrainer:
    ...
    def train(
        ...
        wandb_logger = WandbLogger(cfg, run_id)
        ...
        # Build training history from logs
        history = self._build_training_history(logs, cfg)

        # Log metrics to wandb if enabled
        wandb_logger.log_metrics(history, cfg.verbose)
    ...
    def _build_training_history(
        ...
        """
        Build training history dictionary from overcomplete logs.
        
        Args:
            logs: Logs dictionary from overcomplete training functions
            config: Training configuration
            
        Returns:
            Dictionary with training history (loss, r2, l1, l0, dead_features_pct)
        """
        ...
        if "z" in logs and logs["z"]:
            n_latents = (
                self.sae.context.n_latents
                if hasattr(self.sae, 'context') and hasattr(self.sae.context, 'n_latents')
                else None
            )
            slow_metrics_freq = config.wandb_slow_metrics_frequency if config.use_wandb else 1
            ...
            for epoch_idx, z_batch_list in enumerate(logs["z"]):
                if isinstance(z_batch_list, list) and len(z_batch_list) > 0:
                    ...
                    if should_compute_slow:
                        l0_val, dead_pct = self._compute_slow_metrics(z_batch_list, n_latents)
                        history["l0"].append(l0_val)
                        history["dead_features_pct"].append(dead_pct)
                    else:
                        # Don't compute slow metrics this epoch - mark as not computed
    ...
    def _compute_slow_metrics(
            self,
            z_batch_list: list[torch.Tensor],
            n_latents: int | None
    ) -> tuple[float, float]:
        """
        Compute slow metrics (L0 and dead features percentage) from z batches.
        
        Args:
            z_batch_list: List of z tensors for an epoch
            n_latents: Number of latent dimensions (for dead features calculation)
            
        Returns:
            Tuple of (l0_value, dead_features_pct)
        """
        l0_vals = []
        all_z_epoch = []
        ...
        l0_value = sum(l0_vals) / len(l0_vals) if l0_vals else 0.0
        ...
        return l0_value, dead_features_pct
