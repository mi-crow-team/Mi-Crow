...
def save_model_activations(
    ...
    - Saves per-batch tensors via the provided Store under runs/{run_name}/batch_XXXXXX.safetensors.
    ...
    handle = model.register_forward_hook_for_layer(layer_signature, save_activations_hook)
    ...
    try:
        with torch.inference_mode():
            for batch_index, texts in enumerate(dataset.iter_batches(batch_size)):
                ...
                enc = model.tokenize(texts, **tok_kwargs)
